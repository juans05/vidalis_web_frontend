...
import { ArrowUp, ArrowDown, Eye, ThumbsUp, Share2, Bookmark } from 'lucide-react';
...
export function AnalyticsDashboard() {
    ...
    const StatCard = ({ title, value, growth, icon: Icon, }) => (<div className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
      <div className="flex items-center justify-between">
        <div>
          ...
          {growth !== undefined && (<div className={`text-sm mt-2 flex items-center gap-1 ${growth >= 0 ? 'text-green-400' : 'text-red-400'}`}>
    ...
    return (<div className="min-h-screen bg-bg-primary p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard title="Total Views" value={stats.totalViews} growth={stats.viewsGrowth} icon={Eye}/>
          <StatCard title="Total Likes" value={stats.totalLikes} growth={undefined} icon={ThumbsUp}/>
          <StatCard title="Total Shares" value={stats.totalShares} growth={undefined} icon={Share2}/>
          <StatCard title="Total Saves" value={stats.totalSaves} growth={undefined} icon={Bookmark}/>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={stats.growthData}>
                ...
                <Legend />
                <Line type="monotone" dataKey="views" stroke="#9D4EDD" dot={false} isAnimationActive={false}/>
                <Line type="monotone" dataKey="followers" stroke="#3A86FF" dot={false} isAnimationActive={false}/>
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={Object.entries(stats.platformBreakdown).map(([name, value]) => ({
            name: name.charAt(0).toUpperCase() + name.slice(1),
            value,
        }))} cx="50%" cy="50%" labelLine={false} label={({ name, value }) => `${name}: ${(value || 0).toLocaleString()}`} outerRadius={80} fill="#8884d8" dataKey="value">
                  {Object.entries(stats.platformBreakdown).map((_, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                </Pie>
        ...
        <div className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
          ...
          <div className="w-full bg-bg-secondary rounded-full h-2 overflow-hidden">
            <div className="bg-primary h-full transition-all" style={{ width: `${(stats.monthlyUsage / stats.monthlyLimit) * 100}%` }}/>
          </div>
