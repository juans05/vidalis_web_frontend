...
import { Lightbulb, TrendingUp, BookOpen, Sparkles, Zap } from 'lucide-react';
export function GrowthProPage() {
    ...
    return (<div className="min-h-screen bg-bg-primary p-8">
      <div className="max-w-7xl mx-auto">
        ...
        ].map(({ key, label, icon: Icon }) => (<button key={key} onClick={() => setActiveTab(key)} className={`flex items-center gap-2 px-4 py-2 rounded-lg transition ${activeTab === key
        ...
        {activeTab === 'insights' && (<div className="space-y-6">
            {insights?.map((insight) => (<div key={insight.id} className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-lg ${insight.impact === 'high'
                  ...
                  <div className="flex-1">
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {insight.actionItems.map((item, idx) => (<li key={idx} className="text-sm text-text-secondary flex items-center gap-2">
        ...
        {/* Best Time Tab */}
        {activeTab === 'besttime' && bestTime && (<div className="space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
                ...
                <div className="flex gap-2 flex-wrap">
                  {bestTime.bestDays.map((day) => (<span key={day} className="px-4 py-2 bg-primary text-white rounded-full font-medium">
                      {day}
                    </span>))}
                </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {bestTime.bestHours.map((hour) => (<span key={hour} className="px-3 py-1 bg-accent/20 text-accent rounded">
                      {hour}:00
                    </span>))}
                </div>
              ...
              <div className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
                ...
                <div className="space-y-2">
                  {Object.entries(bestTime.engagementByDay).map(([day, engagement]) => (<div key={day} className="flex items-center gap-2">
                      <span className="w-20 text-text-muted text-sm">{day}</span>
                      <div className="flex-1 bg-bg-secondary h-2 rounded overflow-hidden">
                        <div className="bg-primary h-full" style={{ width: `${(engagement / 12000) * 100}%` }}/>
                      </div>
        ...
        {/* Strategy Tab */}
        {activeTab === 'strategy' && (<div className="space-y-6">
            {strategy?.map((item) => (<div key={item.topic} className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <ul className="space-y-2">
                      {item.recommendations.map((rec, idx) => (<li key={idx} className="text-text-secondary text-sm flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      {item.examples.map((example, idx) => (<li key={idx} className="text-text-secondary text-sm flex items-start gap-2">
        ...
        {/* Viral Trends Tab */}
        {activeTab === 'viral' && viralHistory && (<div className="bg-bg-card rounded-lg p-6 shadow-lg border border-bg-tertiary">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={viralHistory}>
                ...
                <YAxis stroke="#A1A1A8" style={{ fontSize: '12px' }}/>
                <Tooltip contentStyle={{ backgroundColor: '#18181D', border: '1px solid #52525B' }} labelStyle={{ color: '#FAFAFA' }}/>
                <Legend />
                <Line type="monotone" dataKey="score" stroke="#9D4EDD" dot={false} isAnimationActive={false}/>
              </LineChart>
            </ResponsiveContainer>
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-bg-secondary p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-primary">
                  {(viralHistory.reduce((sum, p) => sum + p.score, 0) / viralHistory.length).toFixed(1)}
                </p>
              </div>
              <div className="bg-bg-secondary p-4 rounded-lg">
...
//# sourceMappingURL=GrowthProPage.js.map